...
var _ = ginkgo.Describe("LaunchSecurity SEV-SNP", func() {
	ginkgo.Context("XML marshalling and unmarshalling", func() {
		ginkgo.It("should marshal SEV-SNP launch security with all fields", func() {
			...
			expectedXML := `<LaunchSecurity type="sev-snp"><policy>0x30000</policy><authorKey>yes</authorKey><vcek>yes</vcek><idAuth>test-id-auth</idAuth><idBlock>test-id-block</idBlock><hostData>test-host-data</hostData><dhCert>test-dh-cert</dhCert><session>test-session</session><cbitpos>51</cbitpos><reducedPhysBits>1</reducedPhysBits></LaunchSecurity>`
			Expect(string(xmlBytes)).To(Equal(expectedXML))
		})

		ginkgo.It("should unmarshal SEV-SNP launch security with all fields", func() {
			xmlData := `<LaunchSecurity type="sev-snp"><policy>0x30000</policy><authorKey>yes</authorKey><vcek>yes</vcek><idAuth>test-id-auth</idAuth><idBlock>test-id-block</idBlock><hostData>test-host-data</hostData><dhCert>test-dh-cert</dhCert><session>test-session</session><cbitpos>51</cbitpos><reducedPhysBits>1</reducedPhysBits></LaunchSecurity>`
	...
	ginkgo.Context("Domain with SEV-SNP launch security", func() {
		ginkgo.It("should marshal domain with SEV-SNP launch security", func() {
			...
			Expect(xmlString).To(ContainSubstring(`<authorKey>yes</authorKey>`))
			Expect(xmlString).To(ContainSubstring(`<vcek>yes</vcek>`))
	...
	ginkgo.Context("SEV vs SEV-SNP differentiation", func() {
		ginkgo.It("should handle regular SEV launch security", func() {
			...
			expectedXML := `<LaunchSecurity type="sev"><policy>0x0001</policy><dhCert>test-dh-cert</dhCert><session>test-session</session></LaunchSecurity>`
			Expect(string(xmlBytes)).To(Equal(expectedXML))
		})
